<?php
/**
 * angelfour functions and definitions
 *
 * @package angelfour
 */
...
function angelfour_scripts() {
	
	wp_enqueue_style( 'angelfour-style', get_stylesheet_uri() );

	wp_enqueue_script( 'angelfour-core-js', get_template_directory_uri() . '/inc/js/compiled.js', array('jquery'), true); 
	
}

add_action( 'wp_enqueue_scripts', 'angelfour_scripts' );
...
function angelfour_social_sharing_buttons($content) {
	global $post;
	if(is_singular() || is_home()){
	
		// Get current page URL 
		$angelfourURL = urlencode(get_permalink());
 
		// Get current page title
		$angelfourTitle = htmlspecialchars(urlencode(html_entity_decode(get_the_title(), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8');
		// $angelfourTitle = str_replace( ' ', '%20', get_the_title());
		
		// Get Post Thumbnail for pinterest
		$angelfourThumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
 
		// Construct sharing URL without using any script
		$twitterURL = 'https://twitter.com/intent/tweet?text='.$angelfourTitle.'&amp;url='.$angelfourURL.'&amp;via=Crunchify';
		$facebookURL = 'https://www.facebook.com/sharer/sharer.php?u='.$angelfourURL;
 
		// Based on popular demand added Pinterest too
		$pinterestURL = 'https://pinterest.com/pin/create/button/?url='.$angelfourURL.'&amp;media='.$angelfourThumbnail[0].'&amp;description='.$angelfourTitle;
...
//add_filter( 'the_content', 'angelfour_social_sharing_buttons');
